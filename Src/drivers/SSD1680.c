...
static void SSD1680_Reset(const EPD_HAL *hal) {
    ...
    hal->gpio_write(hal->user_data, EPD_PIN_RESET, true);
    hal->delay_ms(hal->user_data, 200);
    SSD1680_WaitUntilIdle(hal, 2000);
}
...
static void SSD1680_setWidthHeight(const EPD_HAL *hal, uint16_t width, uint16_t height){
    ...
    SSD1680_WriteData(hal, (width_bytes - 1) & 0xFF);
...
static void SSD1680_WriteRamBegin(const EPD_HAL *hal, uint8_t color_layer){
...
static void SSD1680_Update(const EPD_HAL *hal){
    ...
    SSD1680_WriteData(hal, 0xF7);
    SSD1680_WriteCommand(hal, SSD1680_MASTER_ACTIVATION);
    SSD1680_WaitUntilIdle(hal, 10000);
}

static void SSD1680_WRITE_RAM(const EPD_HAL* hal, const uint8_t* data, uint32_t length){
    for(uint32_t i = 0; i < length; i++){
        SSD1680_WriteData(hal, data[i]);
    }
}

static void SSD1680_init_extended(EPD_HAL* hal){
    SSD1680_setDataEntryMode(hal, 0x03);
}
...
const EPD_Driver_Interface SSD1680_Driver = {
    ...
    .init = SSD1680_init_extended,
    .update = SSD1680_Update,
